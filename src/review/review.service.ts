...
export class ReviewService {
  ...
  private async validateReviewOwner(userId: string, reviewId: string) {
    ...
    if (!review) throw new NotFoundException('요청한 리소스를 찾을 수 없습니다.');
    if (review.userId !== userId) throw new UnauthorizedException('인증이 필요합니다.');
    return review;
  }

  // Review 등록
  async createReview(createReviewDto: CreateReviewDto & { userId: string; productId: string }) {
    const { userId, productId, rating, content } = createReviewDto;
    const order = await this.reviewRepository.findOrderByCondition(userId, productId);
    if (!order)
      throw new UnauthorizedException('해당 상품을 구매한 사용자만 리뷰를 작성할 수 있습니다.');

    const existingReview = await this.reviewRepository.findReviewByCondition(userId, productId);

    if (existingReview) throw new UnauthorizedException('이미 리뷰를 작성한 상품입니다.');

    return this.reviewRepository.createReview(userId, productId, rating, content);
  }

  // Product에 해당하는 Review 목록 조회
  async findAllByProductId(productId: string, query: { limit: number; page: number }) {
